...
class Generator_v_de(keras.Model):
    def __init__(self, filters, output_shape, repeat_num=0, num_cov=4, last_kernel_size=3, kernel_size=3, repeat=0, concat=False):
        ...
        assert(self.repeat_num > 0 and np.sum(
            [i % np.power(2, self.repeat_num-1) for i in output_shape[:-1]]) == 0)
        ...
        self.batch_norm_list = [keras.layers.BatchNormalization(
        ) for _ in range(self.repeat_num * self.num_conv)]
